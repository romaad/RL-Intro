...
class _CNNNetworkSeparateHeads(nn.Module):
    """CNN backbone with separate output heads for different action phases.
    ...
    Architecture::
        ...
        Other block  (D,)
           ...
           Linear(h2 → h3) + ReLU            ┘
            ├──────────────────────────────────┐
            ↓                                  ↓
           Linear(h3 → 1)  [head_bid]   Linear(h3 → 1)  [head_play]
    """
    ...
    def __init__(
        ...
        self.head_bid = nn.Linear(h3, 1)   # bidding-phase actions
        self.head_play = nn.Linear(h3, 1)  # playing-phase actions
        ...
        nn.init.kaiming_uniform_(self.conv.weight, nonlinearity="relu")
        nn.init.zeros_(self.conv.bias)
        for layer in (self.fc1, self.fc2, self.fc3, self.head_bid, self.head_play):
            nn.init.kaiming_uniform_(layer.weight, nonlinearity="relu")
            nn.init.zeros_(layer.bias)
    ...
    def forward(
        ...
        Args:
            cnn_block:    shape (1, C, L)  – batched channel-first card features
            other_block:  shape (1, D)     – batched scalar features
            use_bid_head: if True, route to the bidding head; else the playing head

        Returns:
            shape (1, 1) – Q-value estimate
        """
        x = self.relu(self.conv(cnn_block))
        x = x.flatten(start_dim=1)
        x = torch.cat([x, other_block], dim=1)
        x = self.relu(self.fc1(x))
        x = self.relu(self.fc2(x))
        x = self.relu(self.fc3(x))
        return self.head_bid(x) if use_bid_head else self.head_play(x)


class CNNSeparateHeadsValueApproximator(ValueApproximator[State, Action]):
    """CNN value function approximator with separate bidding and playing output heads.

    Shares the same convolutional backbone for all actions, but routes
    to specialised output heads depending on the action type.  Which head
    to use is determined by the *bid_head_selector* callable supplied at
    construction time.

    Sharing the same instance across multiple agents lets them all train
    the same underlying network (parameter sharing).
    """
    ...
    def __init__(
        ...
        cnn_channel_slices: list[tuple[int, int]],
        other_slices: list[tuple[int, int]],
        bid_head_selector: Callable[[Action], bool],
        cnn_filters: int = 16,
        cnn_kernel: int = 4,
        fc_hidden: tuple[int, int, int] = (256, 128, 64),
        alpha: float = 0.001,
        ...
        self._cnn_channel_slices = cnn_channel_slices
        self._other_slices = other_slices
        self._bid_head_selector = bid_head_selector

        num_channels = len(cnn_channel_slices)
        other_dim = sum(stop - start for start, stop in other_slices)

        self._net = _CNNNetworkSeparateHeads(
            cnn_input_len=cnn_input_len,
            num_channels=num_channels,
            other_dim=other_dim,
            cnn_filters=cnn_filters,
    ...
    def predict(self, s: State, a: Action) -> float:
        features = self._feature_extractor(s, a)
        cnn_block, other_block = self._build_tensors(features)
        use_bid = self._bid_head_selector(a)
        with torch.no_grad():
            q = self._net(cnn_block, other_block, use_bid_head=use_bid)
        return float(q.item())
...
class CNNSeparateHeadsApproxAgent(QAgent[State, Action]):
    """Agent using a CNN with separate bidding and playing policy heads."""

    _approximator: CNNSeparateHeadsValueApproximator[State, Action]
    _epsilon: float

    def __init__(
        self,
        value_approximator: CNNSeparateHeadsValueApproximator[State, Action],
        epsilon: float = 0.05,
    ) -> None:
        self._approximator = value_approximator
        self._epsilon = epsilon
    ...
    def checkpoint(self) -> None:
        path = f"{self.PICKLE_PATH}/{self.name}_sep_heads_checkpoint.pt"
        os.makedirs(os.path.dirname(path), exist_ok=True)
        torch.save(self.get_state(), path)
